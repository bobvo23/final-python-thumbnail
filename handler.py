...
def upload_to_s3(bucket, key, image, img_size):
    ...
    dynamo_log = s3_save_thumbnail_url_to_dynamo(
        url_path=url, img_size=img_size)
    print(dynamo_log)
    return url
...
def s3_thumbnail_generator(event, context):
    ...
    if (not key.endswith("_thumbnail.png")):
        image = get_s3_s3_image(bucket, key)
    ...
    body = {
        "mes`sage": "Go Serverless v3.0! Your function executed successfully!",
        "input": event,
    }
...
def s3_delete_item(event, context):
    # todo checking whether the id is in dynamodb table
